...
func calculateAmount(quantity, buyPrice, sellPrice, buyFees, sellFees float64) float64 {
	...
	bf := int64(buyFees * scale)
	sf := int64(sellFees * scale)
	// Integer-Berechnung
	diff := sp - bp
	amount := (q * diff) / scale
	fees := (bf + sf)
	amount -= fees // Gebühren abziehen
